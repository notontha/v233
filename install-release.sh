...
get_version() {
    ...
    if [[ -z "$VERSION" ]]; then
        ...
        if ! "curl" ${PROXY} -o "$TMP_FILE" 'https://api-github.notontha.workers.dev/repos/v2fly/v2ray-core/releases/latest'; then
            "rm" "$TMP_FILE"
...
download_v2ray() {
    ...
    DOWNLOAD_LINK="https://github.notontha.workers.dev/v2fly/v2ray-core/releases/download/$RELEASE_VERSION/v2ray-linux-$MACHINE.zip"
